...
async function sauvegarderNouveauPseudo() {
  const input = document.getElementById('nouveau-pseudo-input');
  const errEl = document.getElementById('nouveau-pseudo-error');
  const pseudo = input?.value.trim();
  if (errEl) errEl.style.display = 'none';

  if (!pseudo || pseudo.length < 2) {
    if (errEl) { errEl.textContent = 'Le pseudo doit faire au moins 2 caractères.'; errEl.style.display = 'block'; }
    return;
  }

  // Vérifier disponibilité
  const { data: dispo } = await db.rpc('pseudo_existe', { p_pseudo: pseudo });
  if (dispo === true) {
    if (errEl) { errEl.textContent = 'Ce pseudo est déjà pris.'; errEl.style.display = 'block'; }
    return;
  }

  const { error } = await db.from('profils').update({ pseudo }).eq('id', compte.userId);
  if (error) {
    if (errEl) { errEl.textContent = 'Une erreur est survenue.'; errEl.style.display = 'block'; }
    return;
  }

  compte.pseudo = pseudo;
  const el = document.getElementById('compte-name');
  if (el) el.textContent = pseudo;
  closeM('modifier-pseudo-popup');
...
async function savePrefs() {
  ...
  await db.from('profils').update({
    ...
    afficher_choix_version:    compte.afficherChoixVersion,
    version_defaut:            compte.versionDefaut
  }).eq('id', compte.userId);
}
...
function _chargerProfil(userId, profil) {
  ...
  if(typeof chargerAchats==='function') chargerAchats().catch(()=>{});
  ...
  compte.role          = profil.role || 'lectrice';
...
/* ══════════════════════════════════════════════════════
   VALIDATION CGU INSCRIPTION
   ══════════════════════════════════════════════════════ */

let _cguLues = false;

function _initCGUScroll() {
  const box = document.getElementById('cgu-scroll-box');
  if (!box) return;
  _cguLues = false;
  // Réinitialiser l'état
  const checks = document.getElementById('cgu-checks');
  const checkCgu = document.getElementById('check-cgu');
  const hint = document.getElementById('cgu-scroll-hint');
  if (checks) checks.classList.add('cgu-checks-disabled');
  if (checkCgu) { checkCgu.disabled = true; checkCgu.checked = false; }
  if (document.getElementById('check-age')) document.getElementById('check-age').checked = false;
  if (hint) hint.classList.remove('hidden');

  box.addEventListener('scroll', function _onScroll() {
    if (_cguLues) return;
    const atBottom = box.scrollTop + box.clientHeight >= box.scrollHeight - 10;
    if (atBottom) {
      _cguLues = true;
      if (checks) checks.classList.remove('cgu-checks-disabled');
      if (checkCgu) checkCgu.disabled = false;
      if (hint) hint.classList.add('hidden');
      box.removeEventListener('scroll', _onScroll);
    }
  });
}

function validerCGUInscription() {
  const checkCgu = document.getElementById('check-cgu');
  const checkAge = document.getElementById('check-age');
  const errEl = document.getElementById('cgu-check-error');

  if (!_cguLues || !checkCgu?.checked || !checkAge?.checked) {
    if (errEl) errEl.style.display = 'block';
    if (!_cguLues) {
      // Scroller automatiquement vers le bas pour indiquer qu'il faut lire
      const box = document.getElementById('cgu-scroll-box');
...
function _lancerApp() {
  ...
  // Flag pour éviter que le guard et le finally se marchent dessus
  let _appLancee = false;

  function _demarrer() {
    if (_appLancee) return;
    _appLancee = true;
    ...
    if (!compte.loggedIn && !sessionStorage.getItem('visiteur_tranche')) {
      setTimeout(() => openModal('compte-conseil-popup'), 400);
    } else if (!compte.loggedIn) {
      compte.trancheAge = sessionStorage.getItem('visiteur_tranche') || 'adulte';
    }
  }

  // Filet de sécurité : si Supabase met trop de temps, on démarre quand même
